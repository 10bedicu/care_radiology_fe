...
export default function RadiologyAuditPopup({
  ...
  return (
    <div className="fixed inset-0 bg-black/40 backdrop-blur-sm flex items-center justify-center z-50">
      <div className="bg-white rounded-xl shadow-lg min-w-4xl max-w-4xl relative p-5 overflow-hidden" style={{width: '900px'}}>
        <button
          onClick={onClose}
        ...
        <h3 className="text-lg font-semibold mb-4">{t("radiology_audit_log")}</h3>

        {/* Created Info */}
        <div className="mb-3 border-b pb-2">
          <p className="text-sm text-gray-500">{t("radiology_created_by")}</p>
          <p className="text-sm font-medium">{formatName(createdLog?.created_by)}</p>
        ...
        {/* Updated Info */}
        <div className="mb-3 border-b pb-2">
          <p className="text-sm text-gray-500">{t("radiology_last_modified_by")}</p>
          <p className="text-sm font-medium">{formatName(latestLog?.updated_by)}</p>
          <p className="text-xs text-gray-400">
        ...
        {/* History Loop */}
<div className="mt-4">
  <p className="font-medium text-sm mb-2">{t("radiology_modification_history")}</p>
            ...
            {log.old_value && log.new_value ? (
              Object.keys(log.new_value).map((field) => (
                <div key={field}>
                  ...
                  <ReactDiffViewer compareMethod={DiffMethod.WORDS} oldValue={log.old_value?.[field] ?? "-"} newValue={log.new_value?.[field] ?? "-"} splitView={false} />
                  </div>
                </div>
              ))
            ) : (
  ...
  <p className="text-gray-400">{t("radiology_no_history_available")}</p>
)}
  </div>
</div>
      </div>
